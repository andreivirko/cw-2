...
var multiItemSlider = (function() {
  return function(selector, config) {
    var _mainElement = document.querySelector(selector), // основный элемент блока
      _sliderWrapper = _mainElement.querySelector('.s5_list'), // обертка для .slider-item
      _sliderItems = _mainElement.querySelectorAll('.s5_item'), // элементы (.slider-item)
      _sliderControls = _mainElement.querySelectorAll('.s5_slider-control'), // элементы управления
      _sliderControlLeft = _mainElement.querySelector('.s5_left'), // кнопка "LEFT"
    ...
    var _transformItem = function(direction) {
      if (direction === 'right') {
        ...
        if (
          _sliderControlRight.classList.contains('s5_right-show') &&
      ...
      if (direction === 'left') {
        ...
        if (
          _sliderControlLeft.classList.contains('s5_right-show') &&
    ...
    var _controlClick = function(e) {
      if (e.target.classList.contains('s5_slider-control')) {
        e.preventDefault();
        var direction = e.target.classList.contains('s5_right')
          ? 'right'
          : 'left';
